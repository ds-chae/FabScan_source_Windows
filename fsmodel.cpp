...
// Following are Mac commands to run the external program "powercrust"...
...
void FSModel::convertPointCloudToSurfaceMesh3()
{
	...
	qDebug() << "cloud_normals->size() = " << cloud_normals->size();
	...
	qDebug("Now doing concatenateFields()");
...
void FSModel::loadPointCloudFromPCD(const std::string &file_name)
{
	...
	if (pcl::io::loadPCDFile(file_name, *pointCloud) == -1) //* load the file
	// Won't load because file is saved as PointXYZ with no RGB?
    {
